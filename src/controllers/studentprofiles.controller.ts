...
export const createStudentDegreePartner = catchAsync(
...
export const getStudentDegreePartner = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    if (!studentId) throw new AppError("Student ID is required", 400);
...
export const updateStudentDegreePartner = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    if (!studentId) throw new AppError("Student ID is required", 400);
    ...
    if (!student) throw new AppError("Student not found", 404);
    if (!student.degree_id)
      throw new AppError("Student does not have a degree partner", 404);
...
export const deleteStudentDegreePartner = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    if (!studentId) throw new AppError("Student ID is required", 400);
...
export const getStudentAcademicDetails = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    if (!studentId) {
...
export const createPersonalDetails = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    ...
    if (!personal_email || typeof personal_email !== "string") {
      throw new AppError(
        "Personal email is required and must be a string.",
        400
      );
    }
    ...
    if (existingDetails) {
      throw new AppError(
        "Personal details already exist for this student.",
        400
      );
    }
...
export const getStudentContactInfo = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
...
export const updateStudentAddress = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
...
export const getPersonalDetails = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    if (!studentId) throw new AppError("Student ID is required.", 400);
...
export const updatePersonalDetails = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    ...
    if (fathers_contact_number !== undefined)
      updateData.fathers_contact_number = fathers_contact_number;
    if (mothers_contact_number !== undefined)
      updateData.mothers_contact_number = mothers_contact_number;
    if (fathers_occupation !== undefined)
      updateData.fathers_occupation = fathers_occupation;
    if (mothers_occupation !== undefined)
      updateData.mothers_occupation = mothers_occupation;
...
export const deletePersonalDetails = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
...
export const createOrUpdateAcademicHistory = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    ...
    const { undergraduate, x_education, xii_education }: AcademicHistoryBody =
      req.body;
    ...
    const validateEducationData = (education: EducationData, type: string) => {
      if (!education.institution || typeof education.institution !== "string") {
        throw new AppError(
          `${type} institution is required and must be a string.`,
          400
        );
      }
      if (!education.degree || typeof education.degree !== "string") {
        throw new AppError(
          `${type} degree is required and must be a string.`,
          400
        );
      }
      if (
        !education.field_of_study ||
        typeof education.field_of_study !== "string"
      ) {
        throw new AppError(
          `${type} field of study is required and must be a string.`,
          400
        );
      }
      if (!education.start_date || !validateDate(education.start_date)) {
        throw new AppError(
          `${type} start date is required and must be a valid date.`,
          400
        );
      }
      ...
      if (
        ...
        (typeof education.grade !== "number" ||
          education.grade < 0 ||
          education.grade > 100)
      ) {
        throw new AppError(
          `${type} grade must be a number between 0 and 100.`,
          400
        );
      }
    };
...
export const deleteAcademicHistoryByEducationId = catchAsync(
  async (req: Request, res: Response) => {
    ...
    if (!educationFound) {
      throw new AppError(
        "Education record not found in academic history for this student.",
        404
      );
    }
...
export const getAllCertifications = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    if (!studentId) throw new AppError("Student ID is required", 400);
...
export const getCertificationById = catchAsync(
  async (req: Request, res: Response) => {
    ...
    if (!certification)
      throw new AppError("Certification not found for this student", 404);
...
export const createCertification = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    if (!studentId) throw new AppError("Student ID is required", 400);
    ...
    if (!name || typeof name !== "string" || name.trim() === "") {
      throw new AppError(
        "Certification name is required and must be a non-empty string",
        400
      );
    }
    if (
      ...
      throw new AppError(
        "Organisation is required and must be a non-empty string",
        400
      );
    }
    if (!start_date || !validateDate(start_date)) {
...
export const updateCertification = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId, certificationId } = req.params;
    if (!studentId) throw new AppError("Student ID is required", 400);
    if (!certificationId)
...
export const deleteCertification = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId, certificationId } = req.params;
    if (!studentId) throw new AppError("Student ID is required", 400);
    if (!certificationId)
...
export const getAllPlacements = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    if (!studentId) throw new AppError("Student ID is required.", 400);
...
export const getPlacementsById = catchAsync(
  async (req: Request, res: Response) => {
    ...
    if (!placement)
      throw new AppError("Placement not found for this student.", 404);
...
export const createPlacements = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    if (!studentId) throw new AppError("Student ID is required.", 400);
    await checkStudentExists(studentId);
    ...
    // Validate
    if (!job_type || !isValidEnumValue(JobType, job_type))
      throw new AppError(
        `job_type required and must be one of: ${Object.values(JobType).join(
          ", "
        )}`,
        400
      );
    if (!work_mode || !isValidEnumValue(WorkMode, work_mode))
      throw new AppError(
        `work_mode required and must be one of: ${Object.values(WorkMode).join(
          ", "
        )}`,
        400
      );
...
export const updatePlacements = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId, placementId } = req.params;
    if (!studentId) throw new AppError("Student ID is required.", 400);
    if (!placementId) throw new AppError("Placement ID is required.", 400);
    ...
    if (job_type !== undefined && !isValidEnumValue(JobType, job_type))
      throw new AppError(
        `job_type must be one of: ${Object.values(JobType).join(", ")}`,
        400
      );
    if (work_mode !== undefined && !isValidEnumValue(WorkMode, work_mode))
      throw new AppError(
        `work_mode must be one of: ${Object.values(WorkMode).join(", ")}`,
        400
      );
    ...
    if (
      ...
      throw new AppError("Company name must be a non-empty string.", 400);
    if (start_date !== undefined && !validateDate(start_date))
      throw new AppError("Start date must be a valid date.", 400);
    if (end_date !== undefined && end_date !== null && !validateDate(end_date))
      throw new AppError("End date must be a valid date.", 400);
...
export const deletePlacements = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId, placementId } = req.params;
    if (!studentId) throw new AppError("Student ID is required.", 400);
    if (!placementId) throw new AppError("Placement ID is required.", 400);
...
export const getAchievementsById = catchAsync(
  async (req: Request, res: Response) => {
    ...
    if (!achievement)
      throw new AppError("Achievement not found for this student", 404);
...
export const createAchievement = catchAsync(
  async (req: Request, res: Response) => {
    ...
    const { title, description, start_date, organisation }: AchievementBody =
      req.body;
...
export const updateAchievements = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId, achievementId } = req.params;
    if (!studentId) throw new AppError("Student ID is required.", 400);
    if (!achievementId) throw new AppError("Achievement ID is required.", 400);
    ...
    if (organisation !== undefined)
      updateData.organisation = organisation ? organisation.trim() : null;
...
export const deleteAchievements = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId, achievementId } = req.params;
    if (!studentId) throw new AppError("Student ID is required.", 400);
    if (!achievementId) throw new AppError("Achievement ID is required.", 400);
...
export const getAllSocialLinks = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    if (!studentId) throw new AppError("Student ID is required.", 400);
...
export const getSocialLinksById = catchAsync(
  async (req: Request, res: Response) => {
    ...
    if (!socialLink)
      throw new AppError("Social link not found for this student", 404);
...
export const createSocialLinks = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId } = req.params;
    if (!studentId) throw new AppError("Student ID is required.", 400);
    ...
    if (
      ...
      throw new AppError(
        `Platform is required and must be one of: ${ALLOWED_PLATFORMS.join(
          ", "
        )}`,
        400
      );
    ...
    if (existing)
      throw new AppError(
        `Social link for platform ${platform} already exists for this student.`,
        400
      );
...
export const deleteSocialLinks = catchAsync(
  async (req: Request, res: Response) => {
    const { studentId, socialLinkId } = req.params;
    if (!studentId) throw new AppError("Student ID is required.", 400);
    if (!socialLinkId) throw new AppError("Social Link ID is required.", 400);
...
export const getStudentProfiles = catchAsync(
  async (req: Request, res: Response<ResponseProfileData>) => {
    const { studentId } = req.params;
