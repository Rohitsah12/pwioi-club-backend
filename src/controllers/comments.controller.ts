...
export const createComment = catchAsync(async (req: Request, res: Response) => {
  ...
  await findUserByRole(id, role);
  const comment = await prisma.comment.create({ data: { post_id: postId, user_id: id, user_role: role, content: content.trim() } });
...
export const updateComment = catchAsync(async (req: Request, res: Response) => {
  ...
  const canEdit = comment.user_id === id || ["SUPER_ADMIN", "ADMIN"].includes(role);
...
export const deleteComment = catchAsync(async (req: Request, res: Response) => {
  ...
  const comment = await prisma.comment.findUnique({ where: { id: commentId } });
  if (!comment) throw new AppError("Comment not found", 404);
  const canDelete = comment.user_id === id || ["SUPER_ADMIN", "ADMIN"].includes(role);
